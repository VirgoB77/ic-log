...
## 本物の清算の目印で確かめ直した結果（無料データ）

Binance の建玉（先物ポジションの総量、1時間ごと、2023年〜今日）が無料で取れたので、
「建玉が1時間で上位1%の減り方をして値下がりした時間」を本物の清算の山とみなして確かめ直した。

- 山のあとに買うと、48時間後の平均は約+0.5%（コスト後）。合図なし（約+0.3%）より上で、勝率56%。
  値動きだけの合図（ほぼ0%）よりはっきり良い。
- 戻りは12〜48時間かけて出る。直後に飛び乗るのではなく、山を確認してから半日〜2日持つ形。
- 上に跳ねてショートが清算された山のあとに売るのは、やはり負ける。
- 期間が上げ相場に偏っている点と、1回あたり0.3〜0.5%でスプレッドが広いと消える点に注意。

詳しくは報告書の「9.」。計算は `compare_liquidations.py`、結果は `out/liq_compare.json` と `out/liq_oi.png`。

## Coinglass から取る（鍵が要る・任意）

Coinglass の API は有料（月29ドル〜）。使わなくても上の無料の建玉データで検証できるので、
以下は「使いたくなったとき」の手順。鍵が入っていない間、取りに行く係は Coinglass の部分を静かに飛ばす。
